...
static int my_dev_probe(struct platform_device *pdev)
{
	...
	if(!device_property_present(dev, "int_var")){
		pr_err("my_dev - int_var poperty is not present \n");
	}	


	if(!device_property_present(dev, "string_var")){
		pr_err("my_dev - string_var poperty is not present \n");
	}

	status = device_property_read_u32(dev, "int_var", &int_var);
	if(status){
		pr_err("my_dev - Error readig int_var property \n");
		return status;
	}	
	...
	if(status){
		pr_err("my_dev - Error readig string_var property \n");
		return status;
	}	
